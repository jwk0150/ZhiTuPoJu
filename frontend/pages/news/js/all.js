 ...
 * -------------------------------------------------------------------------
 * 路由：all.html
 * 功能：展示全部资讯（内部深度报道 + 行业快讯外链），顶部含搜索栏（关键词 /
 *       分类筛选），底部含翻页器。内部新闻整行点击进详情，外链新闻整行跳转原文。
 * ========================================================================= */
(function () {
  'use strict';
  ...
  var PER_PAGE = 8; /* 每页条数；29 条 → 4 页，足以演示翻页 */
  ...
  function render(opts) {
    ...
    var tabsEl = document.getElementById('all-tabs');
    ...
    var pagerEl = document.getElementById('all-pager');
    ...
    if (opts.scroll) {
      var sec = document.getElementById('all-section');
  ...
  /* ---------- 交互绑定 ---------- */
  function bind() {
    var kwEl = document.getElementById('all-kw');
    var clearEl = document.getElementById('all-clear');
    ...
    /* 分类筛选 + 翻页 */
    var section = document.getElementById('all-section');
    if (section) {
    ...
    /* 内部资讯（data-go）→ 详情页；外链（<a> 自带 href）直接跳转 */
  ...
  /* ---------- 启动 ---------- */
  function init() {
    var upd = document.getElementById('all-update');
