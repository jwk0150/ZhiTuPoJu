...
(async () => {
  ...
  // denser sample left half where cat lives - crop later by full page
  const times = [];
  for (let t = 0; t <= dur - 0.05; t += 0.5) times.push(Number(t.toFixed(2)));
  for (const t of times) {
    ...
    // crop left side where cat is
    await p.screenshot({
      path: path.join(out, `cat-${String(t).replace('.', '_')}.png`),
      clip: { x: 0, y: 280, width: 420, height: 320 }
    });
  }
  console.log('frames', times.length, 'dur', dur);
  await b.close();
