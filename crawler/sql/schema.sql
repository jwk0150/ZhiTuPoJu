-- ============================================================
-- 执图破局 — 智联招聘数据爬取库 Schema
-- 数据库：zhilian_crawl_db  ·  PostgreSQL 15+  ·  UTF8
-- 说明：本文件不含 CREATE DATABASE / \c，由 init_db.py 负责建库后连入执行。
--       两张表 1:1 —— job_postings（查询总表） + job_posting_details（细节表）
-- ============================================================

-- ---------- 扩展 ----------
CREATE EXTENSION IF NOT EXISTS pg_trgm;    -- 三元组模糊搜索（标题/公司）
CREATE EXTENSION IF NOT EXISTS btree_gin;  -- 让 GIN 支持 B-tree 类型（复合索引）
CREATE EXTENSION IF NOT EXISTS pgcrypto;   -- SHA256 指纹（digest）
...
-- 索引
CREATE INDEX IF NOT EXISTS idx_job_postings_title     ON job_postings USING btree (job_title);
CREATE INDEX IF NOT EXISTS idx_job_postings_company   ON job_postings USING btree (company_name);
CREATE INDEX IF NOT EXISTS idx_job_postings_city      ON job_postings USING btree (city);
CREATE INDEX IF NOT EXISTS idx_job_postings_publish   ON job_postings USING btree (publish_time DESC);
CREATE INDEX IF NOT EXISTS idx_job_postings_crawl     ON job_postings USING btree (crawl_time DESC);
CREATE INDEX IF NOT EXISTS idx_job_postings_status    ON job_postings USING btree (status);
CREATE INDEX IF NOT EXISTS idx_job_postings_salary    ON job_postings USING btree (salary_min, salary_max);
CREATE INDEX IF NOT EXISTS idx_job_postings_exp       ON job_postings USING btree (experience);
CREATE INDEX IF NOT EXISTS idx_job_postings_edu       ON job_postings USING btree (education);
CREATE INDEX IF NOT EXISTS idx_job_postings_title_trgm   ON job_postings USING gin (job_title gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_job_postings_company_trgm ON job_postings USING gin (company_name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_job_postings_city_pub  ON job_postings USING btree (city, publish_time DESC);
CREATE INDEX IF NOT EXISTS idx_job_postings_comp_gin  ON job_postings USING gin (status, city, experience);

-- 注释
COMMENT ON TABLE  job_postings IS '招聘数据查询总表 —— 存储爬取岗位的汇总信息，用于前台列表、筛选、排序';
COMMENT ON COLUMN job_postings.id             IS '主键ID';
COMMENT ON COLUMN job_postings.source_name    IS '来源平台名称（zhilian/boss/liepin/lagou/51job 等）';
...
-- ============================================================
-- 二、细节表 job_posting_details（与 job_postings 1:1）
-- ============================================================
CREATE TABLE IF NOT EXISTS job_posting_details (
    ...
    extra                JSONB,                                -- 平台特有字段
    raw_html             TEXT,                                 -- 原始HTML/JSON（备份留底）
...
-- 索引
CREATE INDEX IF NOT EXISTS idx_detail_job_id    ON job_posting_details USING btree (job_id);
CREATE INDEX IF NOT EXISTS idx_detail_industry  ON job_posting_details USING btree (company_industry);
CREATE INDEX IF NOT EXISTS idx_detail_cat_l1    ON job_posting_details USING btree (job_category_l1);
CREATE INDEX IF NOT EXISTS idx_detail_cat_l2    ON job_posting_details USING btree (job_category_l2);
CREATE INDEX IF NOT EXISTS idx_detail_skills    ON job_posting_details USING gin (skills);
CREATE INDEX IF NOT EXISTS idx_detail_benefits  ON job_posting_details USING gin (benefits);
CREATE INDEX IF NOT EXISTS idx_detail_keywords  ON job_posting_details USING gin (keywords);
CREATE INDEX IF NOT EXISTS idx_detail_labels    ON job_posting_details USING gin (job_labels);
CREATE INDEX IF NOT EXISTS idx_detail_extra     ON job_posting_details USING gin (extra);
CREATE INDEX IF NOT EXISTS idx_detail_fulltext  ON job_posting_details USING gin (
...
COMMENT ON COLUMN job_posting_details.extra              IS '平台特有字段（JSONB）';
COMMENT ON COLUMN job_posting_details.raw_html           IS '原始HTML/JSON备份';
...
DROP TRIGGER IF EXISTS trg_generate_fingerprint ON job_postings;
...
DROP TRIGGER IF EXISTS trg_calculate_completeness ON job_postings;
...
DROP TRIGGER IF EXISTS trg_touch_updated_at ON job_posting_details;
CREATE TRIGGER trg_touch_updated_at
    BEFORE UPDATE ON job_posting_details
    FOR EACH ROW EXECUTE FUNCTION fn_touch_updated_at();

COMMENT ON FUNCTION fn_touch_updated_at IS '更新细节表时自动刷新 updated_at';

-- ============================================================
-- 四、统计视图 v_collection_stats（后台采集 KPI）
-- ============================================================
