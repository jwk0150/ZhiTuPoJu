...
async def upsert_seed_jobs(conn: asyncpg.Connection, city_short: str, jobs: list[dict]) -> int:
    ...
    for j in jobs:
        ...
        exists = await conn.fetchval(
            "SELECT 1 FROM the_total_table WHERE source_name='ai_seed' AND source_id=$1",
            sid,
        )
        if exists:
            continue
...
async def delete_all_ai_seed(conn: asyncpg.Connection) -> int:
    deleted = await conn.execute("DELETE FROM the_total_table WHERE source_name='ai_seed'")
    try:
        return int(deleted.split()[-1])
    except Exception:
        return 0
...
async def ensure_city_min_jobs(conn: asyncpg.Connection, city_short: str, min_jobs: int = 20) -> int:
    ...
    sids = await conn.fetch(
        "SELECT source_id FROM the_total_table WHERE source_name='ai_seed' AND split_part(city,'·',1)=$1",
        city_short,
    )
...
async def main() -> None:
    ...
    try:
        await migrate(conn)
        print("[迁移] the_total_table 已补齐 industry_tags/skills/job_description/qualification/work_experience/city_seed/sort_weight 列")
